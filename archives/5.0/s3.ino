...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pChar) {
        ...
        if (data == "ME_START") { 
            ...
            if(f) { f.print(data.substring(8)); f.close(); }
        }
    ...
    if (keyName == "ME") {
        if (FFat.exists("/me_hex.txt")) {
            ...
            if (f) {
                Keyboard.print("[HEXS]"); 
                delay(20);
                while (f.available()) { Keyboard.print((char)f.read()); delay(1); }
                f.close();
                delay(20); Keyboard.print("[HEXE]");
                return; 
            }
        }
    }
                    ...
                    if (currentState) {
                        ...
                        if (keycode == K_FN) {
                            ...
                            if (keycode == K_LOGO && fnPressed) {
                                Serial.println("检测到 Fn + LOGO，执行重启");
                                ...
                                if (fnPressed) {
                                    ...
                                    Keyboard.press(' ');
                                    Keyboard.release(' ');
                                } else ConsumerControl.press(CONSUMER_CONTROL_SCAN_NEXT_TRACK);
                            } else if (keycode == K_PREV) {
                                if (fnPressed) {
                                    SystemControl.press(SYSTEM_CONTROL_POWER_OFF);
                                    SystemControl.release();
                                } else ConsumerControl.press(CONSUMER_CONTROL_SCAN_PREVIOUS_TRACK);
...
void loop() {
    ...
    while (Serial1.available()) {
        ...
        if (cmd == "V+") {
            ...
            Serial.println("收到C3串口指令，执行重启 (REBOOT)...");
            delay(100);
            esp_restart();
        } else if (cmd == "N_S3_ROOT") {
            Serial.println("收到C3串口指令，进入强制刷机模式 (ROOT)...");
            delay(100);
