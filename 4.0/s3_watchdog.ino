...
// ================= 看门狗状态监控 =================
static bool wdtTriggered = false;  // 看门狗是否触发过
...
void initKeyMatrix() {
    ...
    keyMatrix[1][3] = K_NEXT; keyMatrix[1][4] = K_PLAY; keyMatrix[1][5] = K_PREV;
    ...
    if (keyName == "ME") {
        if (FFat.exists("/me_hex.txt")) {
            ...
            if (f) {
                ...
                int sentCount = 0;  // 计数器，每50个字符喂一次狗
                while (f.available()) {
...
// ================= I2C 自主恢复函数 =================
void recoverI2CBus() {
    Serial.println("检测到 I2C 故障，正在尝试自我恢复...");
    Wire.end();
    delay(10);
                        ...
                        if (keycode == K_FN) {
                            ...
                            if (keycode == K_PLAY || keycode == K_NEXT || keycode == K_PREV) {
                                ...
                                if (pressedDuration >= 2000 && pressedDuration < 8000) {
                                    ...
                                    Serial.println("检测到长按 LOGO 2S~8S 释放，普通重启...");
                                    delay(200);
                                    esp_restart();
                                }
...
void setup() {
    ...
    // ========== 看门狗初始化 ==========
    // 参数说明：
    // - 第一个参数：超时时间（秒），10秒足够完成任何正常操作
    // - 第二个参数：true=超时后自动重启，false=只触发中断
    esp_task_wdt_init(10, true);

    // 可选：注册看门狗超时回调（需要在 init 之前设置）
    // esp_task_wdt_isr_user_handler_register(wdt_isr_handler, NULL);

    // 将当前任务添加到看门狗监控
    esp_task_wdt_add(NULL);

    Serial.println("看门狗已启用（10秒超时）");
    Serial.println("如果程序卡死超过10秒，将自动重启");
    // ===================================
    ...
    if (!mcp.begin_I2C(MCP23017_ADDR, &Wire)) {
        Serial.println("MCP23017 初始化失败，尝试急救...");
        recoverI2CBus();
    }
    ...
    if (!FFat.begin(true)) {
        Serial.println("警告：FFat(FATFS) 挂载失败！");
    } else {
...
void loop() {
    ...
    if (lastState[1][6]) {
        ...
        if (pressedDuration >= 8000) {
            Serial.println("检测到长按 LOGO 达到 8 秒，进入 ROM 刷机模式...");
